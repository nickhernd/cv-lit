...
<template>
  <div class="space-y-6">
    <div class="flex justify-between items-center border-b border-slate-200 pb-4">
      <h1 class="text-2xl font-bold text-slate-900">Análisis Geoespacial de Costa</h1>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-4 gap-6">
      ...
      <div class="lg:col-span-3 space-y-4">
        ...
        <div v-if="selectedCamId" class="card-standard p-4 bg-slate-50 flex items-center space-x-4">
           <label class="text-xs font-bold text-slate-500 uppercase">Seleccionar Fotograma:</label>
      ...
      <aside class="space-y-6">
        <div v-if="result" class="card-standard overflow-hidden">
           ...
           <div class="p-6 space-y-6">
              <div>
                <p class="text-[10px] font-bold text-slate-400 uppercase mb-2">Área Seca Estimada</p>
                <div class="flex items-baseline space-x-1">
                  <p class="text-4xl font-bold text-slate-900">{{ result.dry_area_m2.toLocaleString() }}</p>
              ...
              <div class="pt-4 border-t border-slate-100 grid grid-cols-1 gap-4">
                <div>
                  <p class="text-[10px] font-bold text-slate-400 uppercase mb-1">Confianza AI</p>
                  <p class="text-lg font-bold text-blue-600">{{ (result.confidence * 100).toFixed(1) }}%</p>
                </div>
                <div>
                  <p class="text-[10px] font-bold text-slate-400 uppercase mb-1">Timestamp</p>
                  <p class="text-xs font-bold text-slate-700">{{ result.timestamp.replace('T', ' ') }}</p>
                </div>
              </div>
        ...
        <div v-else class="card-standard p-8 text-center bg-slate-50 border-dashed">
           ...
           <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest leading-relaxed">Ejecute el análisis para obtener métricas geoespaciales</p>
        </div>
      </aside>
    </div>
    ...
    <div v-if="analyzing" class="fixed inset-0 z-[200] bg-slate-900/10 flex flex-col items-center justify-center">
       <div class="bg-white border border-slate-200 p-6 rounded shadow-xl flex items-center space-x-4">
          ...
          <p class="text-sm font-bold text-slate-700 uppercase tracking-widest">Procesando Segmentación...</p>
       </div>
    </div>
  </div>
</template>
