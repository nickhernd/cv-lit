...
\begin{infobox}[¿Sobre qué se ejecuta: una imagen, un lote, o toda la cámara?]
...
\begin{tikzpicture}[node distance=0.5cm and 1.4cm]
  ...
  \node[block, fill=Orange!15, right=of desc] (base) {2. Base\\{\tiny imagen más antigua}};
...
\begin{enumerate}[leftmargin=1.5em]
  ...
  \item \textbf{Selección de base}: la imagen descargada con el timestamp de captura
        \textbf{más antiguo} del lote se usa como referencia para la alineación — el resto del
        lote se alinea contra ella. Si el lote tiene una sola imagen, este y el siguiente paso
        se omiten (nada que alinear).
